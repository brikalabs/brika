...
export function setupSparks(core: ContextCore) {
  ...
  return {
    methods: {
      registerSpark(spark: { id: string; schema?: Record<string, unknown> }): {
        id: string;
      } {
        const { id } = spark;
        ...
        client.send(registerSparkMsg, {
          spark: {
            id,
            schema: spark.schema as Record<string, Json> | undefined,
          },
        });
        return {
          id,
      ...
      subscribeSpark(sparkType: string, handler: (event: SparkEvent) => void): () => void {
        ...
        return () => {
          ...
          client.send(unsubscribeSparkMsg, {
            subscriptionId,
          });
        };
      },
