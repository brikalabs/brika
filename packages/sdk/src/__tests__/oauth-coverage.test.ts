...
function getStoredToken(key: string): Record<string, unknown> {
  ...
  throw new Error(`Expected stored token at "${key}" to be an object, got ${typeof val}`);
}
...
describe('OAuth coverage: authenticated fetch', () => {
  ...
  test('fetch throws when token is expired and refresh throws network error', async () => {
    ...
    globalThis.fetch = mockFetch(() => Promise.reject(new Error('DNS resolution failed')));
