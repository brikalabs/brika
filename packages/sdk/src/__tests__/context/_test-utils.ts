...
export interface TestHarness {
  ...
  sentMessages: Array<{ name: string; payload: unknown }>;
...
export function createTestHarness(manifest?: Partial<Manifest>): TestHarness {
  ...
  const sentMessages: Array<{ name: string; payload: unknown }> = [];
  const logMessages: Array<{ level: string; message: string }> = [];

  const client = {
    send: mock((def: { name: string }, payload: unknown) => {
      sentMessages.push({ name: def.name, payload });
    }),
    on: mock((def: { name: string }, handler: Handler) => {
      onHandlers.set(def.name, handler);
      return () => onHandlers.delete(def.name);
    }),
    implement: mock((def: { name: string }, handler: Handler) => {
  ...
  const core: ContextCore = {
    ...
    manifest: { name: 'test-plugin', version: '1.0.0', ...manifest },
  ...
  function callImpl(name: string, input: unknown) {
    ...
    if (!handler) throw new Error(`No implementation registered for "${name}"`);
