...
describe('BunMock', () => {
  ...
  describe('fs()', () => {
    test('creates virtual filesystem from file paths (auto-infers directories)', async () => {
      ...
      // Files
      expect(await Bun.file('/config.json').json()).toEqual({
        port: 3000,
      });
      expect(await Bun.file('/locales/en/common.json').json()).toEqual({
        greeting: 'Hello',
      });
  ...
  describe('file()', () => {
    test('mocks file with JSON content', async () => {
      ...
      expect(await Bun.file('/test.json').json()).toEqual({
        key: 'value',
      });
    });
  ...
  describe('write()', () => {
    test('updates virtual fs', async () => {
      ...
      expect(await Bun.file('/new.json').json()).toEqual({
        created: true,
      });
      expect(bun.hasFile('/new.json')).toBe(true);
      expect(bun.getFile('/new.json')).toMatchObject({
        created: true,
      });
    });
  });
  ...
  describe('real world examples', () => {
    test('i18n service', async () => {
      ...
      for await (const dir of new Bun.Glob('*/').scan({
        cwd: '/app/locales',
      })) {
      ...
      expect(await Bun.file('/app/locales/fr/common.json').json()).toEqual({
        hello: 'Bonjour',
      });
    });
    ...
    test('plugin registry', async () => {
      bun
        .fs({
          ...
          '/plugins/node_modules/@test/plugin/package.json': {
            version: '1.2.3',
          },
        })
        .spawn({
...
describe('useBunMock', () => {
  ...
  test('resets between tests', async () => {
    ...
    bun
      ...
      .apply();
    expect(await Bun.file('/other.json').json()).toEqual({
      data: true,
    });
  });
