 ...
 * Lives in `@brika/schema` (the leaf package) so the compiler's shim
 * needs no `@brika/sdk` import; the SDK re-exports it unchanged as
 * `@brika/sdk/grants/fs-runtime`.
 *
 * The shapes here MUST stay in sync with the Zod schemas in the SDK's
 * `grants/fs`. That synchronisation is enforced structurally: that
 * module exports `BrikaFsRuntime` from this file, and the `Ctx['fs']`
 * augmentation (also there) references the same type, so any drift
 * between the schemas and the runtime contract surfaces at typecheck
 * time on the grant spec.
 */
