    ...
    z
      .array(
        z
          ...
          .regex(
            /^[A-Za-z_$][\w$]*$/,
            'field names are zod object keys (identifiers); they interpolate into i18n keys'
          )
      )
      .describe(
        "Config field names from the block's zod config schema, generated by `brika build`. The workflow editor labels them through i18n (`fields.<name>.label`), so declaring them here makes that key set statically checkable."
      )
  ),
});
...
/**
 * A server action entry, generated by `brika build` from a static source scan
 * (never hand-written). `id` is the RPC hash the server build injects
...
export const ActionSchema = z.object({
  id: z
    ...
    .describe('Action RPC id (build-time hash of file + export name)'),
  file: z
    .string()
    .min(1)
    .refine(
    ...
    .describe('Source file defining the action, relative to the plugin root'),
  name: z.string().min(1).describe('Exported binding name (`default` for a default export)'),
});
...
export const PluginPackageSchema = BasePackageJson.extend({
  ...
  actions: z.optional(
    z
      .array(ActionSchema)
      .describe(
        'Server actions (internal RPC endpoints callable from plugin pages/bricks), generated by `brika build`. The hub refuses to dispatch an action id not declared here.'
      )
  ),
  ...
  readme: z.optional(
    z
      ...
      .describe(
        'Localized README paths keyed by language tag, e.g. { "en": "README.md", "fr": "README.fr.md" }'
      )
  ),
