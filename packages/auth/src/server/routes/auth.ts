...
import { route, rateLimit } from '@brika/router';
...
const login = route.post({ path: '/login', middleware: [rateLimit({ window: 60, max: 5 })], body: LoginSchema, handler: async (ctx) => {
...
const logout = route.post({ path: '/logout', handler: async (ctx) => {
  const authService = inject(AuthService);
...
const sessionInfo = route.get({ path: '/session', handler: async (ctx) => {
  const session = requireSession(ctx);
  const userService = inject(UserService);
  const user = await userService.getUser(session.userId);

  return { user, scopes: session.scopes };
}});
