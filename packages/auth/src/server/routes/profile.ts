...
import { route, rateLimit, BadRequest } from '@brika/router';
import { inject } from '@brika/di';
import { z } from 'zod';
import { PasswordSchema, NameSchema } from '../../schemas';
import { UserService } from '../../services/UserService';
import { SessionService } from '../../services/SessionService';
import { requireSession } from '../requireSession';
import { serveImage, ImageQuerySchema } from '../serveImage';
...
const updateProfile = route.put({ path: '/profile', body: UpdateProfileSchema, handler: async (ctx) => {
  const session = requireSession(ctx);
  const userService = inject(UserService);
  const user = await userService.updateUser(session.userId, { name: ctx.body.name });
  return { user };
}});

/** PUT /profile/avatar — Upload avatar image */
const uploadAvatar = route.put({ path: '/profile/avatar', handler: async (ctx) => {
  const session = requireSession(ctx);
  const contentType = ctx.req.headers.get('content-type') ?? '';
  let imageBuffer: Buffer;

  if (contentType.includes('application/json')) {
    const body = (ctx.body ?? {}) as { data: string };
    if (!body.data) throw new BadRequest('Missing image data');
    imageBuffer = Buffer.from(body.data, 'base64');
  } else {
    imageBuffer = Buffer.from(await ctx.req.arrayBuffer());
  }

  if (imageBuffer.length === 0) throw new BadRequest('Empty image data');
  if (imageBuffer.length > 5 * 1024 * 1024) throw new BadRequest('Image too large (max 5MB)');
  if (!isValidImage(imageBuffer)) throw new BadRequest('Invalid image format (PNG, JPEG, or WebP required)');

  const userService = inject(UserService);
  const avatarHash = await userService.setAvatar(session.userId, imageBuffer);
  return { ok: true, avatarHash };
}});

/** DELETE /profile/avatar — Remove avatar */
const removeAvatar = route.delete({ path: '/profile/avatar', handler: async (ctx) => {
  const session = requireSession(ctx);
  const userService = inject(UserService);
  await userService.removeAvatar(session.userId);
  return { ok: true };
}});

/** PUT /profile/password — Change own password */
const changePassword = route.put({ path: '/profile/password', middleware: [rateLimit({ window: 900, max: 10 })], body: ChangePasswordSchema, handler: async (ctx) => {
  const session = requireSession(ctx);

  const userService = inject(UserService);
  const valid = await userService.verifyPassword(session.userId, ctx.body.currentPassword);
  if (!valid) throw new BadRequest('Invalid current password');
...
const getAvatar = route.get({ path: '/avatar/:userId', query: ImageQuerySchema, handler: async (ctx) => {
  const userService = inject(UserService);
