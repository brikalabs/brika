...
export class PhotonPipeline {
  ...
  private resizeOpt: { width?: number; height?: number; fit: ResizeFit } | null = null;
  private outputFormat: OutputFormat = { format: 'webp' };
  ...
  resize(width: number, height: number, options?: { fit?: ResizeFit }): this;
  resize(options: ResizeOptions): this;
  resize(
    widthOrOptions: number | ResizeOptions,
    height?: number,
    options?: { fit?: ResizeFit },
  ): this {
    if (typeof widthOrOptions === 'number') {
      this.resizeOpt = { width: widthOrOptions, height, fit: options?.fit ?? 'contain' };
    } else {
      const { width, height, fit } = widthOrOptions;
      this.resizeOpt = { width, height, fit: fit ?? 'contain' };
    }
    return this;
  }
  ...
  webp(): this {
    this.outputFormat = { format: 'webp' };
    return this;
  }

  /** Output as JPEG. */
  jpeg(options?: JpegOptions): this {
    this.outputFormat = { format: 'jpeg', quality: options?.quality ?? 80 };
    return this;
  }

  /** Output as PNG. */
  png(): this {
    this.outputFormat = { format: 'png' };
    return this;
  }
  ...
  toBuffer(): Buffer {
    ...
    const op = this.resizeOpt
      ? computeResize(width, height, this.resizeOpt.width, this.resizeOpt.height, this.resizeOpt.fit)
      : null;
