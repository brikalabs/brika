...
describe('Source code coverage', () => {
  it("every `new BrikaError('CODE', ...)` literal in src is in the catalog", async () => {
    ...
    // path component.
    const SKIP_SEGMENTS = new Set(['node_modules', 'dist', '__benchmarks__', '.bun']);
    const glob = new Bun.Glob(`{${SEARCH_DIRS.join(',')}}/**/*.{ts,tsx}`);
    const codes = new Set<string>();
    for await (const relPath of glob.scan({ cwd: REPO_ROOT, onlyFiles: true })) {
      // Tests are colocated next to source (see TESTING.md) — they may
      // reference codes that are intentionally not in the catalog
      // (PLUGIN_DEFINED_CODE-style fixtures).
