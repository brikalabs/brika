/**
 * `@brika/embed` — build-time asset-embedding macros. Pack repo
 * directories (UI `dist`, templates, locale JSON) into gzip-compressed
 * tar archives inlined into the single hub binary via
 * `import { loadTarBytes } from '@brika/embed' with { type: 'macro' }`.
 *
 * These run at build time and read the repo's on-disk layout, so their
 * repo-structure-dependent guard branches (no `packages/` dir, unreadable
 * `package.json`) can't be reproduced in a unit test. The package is
 * coverage-exempt (see `bunfig.toml` / `scripts/check-coverage.ts`); the
 * archives are validated end-to-end by the hub build and the tests here.
 */
