...
/**
 * Each CLI invocation forks a fresh Bun runtime (~130ms on a warm box).
 * Tests run concurrently and each owns its own tmpdir so the spawns
 * overlap — wall-clock = the slowest single spawn, not the sum.
 */
async function withWorkDir<T>(body: (workDir: string) => Promise<T>): Promise<T> {
  const workDir = await mkdtemp(join(tmpdir(), 'mortar-cli-'));
  try {
...
describe.concurrent('mortar CLI', () => {
