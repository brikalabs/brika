...
// .wrangler/tmp/bundle-3pP1yl/checked-fetch.js
...
// src/utils.ts
var CDN_URLS = {
  unpkg: /* @__PURE__ */ __name((pkg, v, p) => `https://unpkg.com/${pkg}${v}${p}`, "unpkg"),
  jsdelivr: /* @__PURE__ */ __name((pkg, v, p) => `https://cdn.jsdelivr.net/npm/${pkg}${v}${p}`, "jsdelivr")
};
var parseVer = /* @__PURE__ */ __name((v) => {
  const p = v.replace(/^\D*/, "").split(".").map(Number);
  return [p[0] ?? 0, p[1] ?? 0, p[2] ?? 0];
}, "parseVer");
var isPreRelease = /* @__PURE__ */ __name((v) => /^\d+\.\d+\.\d+-/.test(v), "isPreRelease");
var satisfiesVersion = /* @__PURE__ */ __name((ver, range) => {
  const [maj, min, pat] = parseVer(ver);
  const r = range.trim();
  if (r.startsWith("^")) {
    const [rMaj2, rMin2, rPat2] = parseVer(r.slice(1));
    return maj === rMaj2 && (min > rMin2 || min === rMin2 && pat >= rPat2);
  }
  if (r.startsWith("~")) {
    const [rMaj2, rMin2, rPat2] = parseVer(r.slice(1));
    return maj === rMaj2 && min === rMin2 && pat >= rPat2;
  }
  if (r.startsWith(">=")) {
    const [rMaj2, rMin2, rPat2] = parseVer(r.slice(2));
    return maj > rMaj2 || maj === rMaj2 && (min > rMin2 || min === rMin2 && pat >= rPat2);
  }
  const [rMaj, rMin, rPat] = parseVer(r);
  return maj === rMaj && min === rMin && pat === rPat;
}, "satisfiesVersion");
var sortVersions = /* @__PURE__ */ __name((versions) => [...versions].sort((a, b) => {
  const [aMaj, aMin, aPat] = parseVer(a);
  const [bMaj, bMin, bPat] = parseVer(b);
  return bMaj - aMaj || bMin - aMin || bPat - aPat;
}), "sortVersions");
var getStableVersions = /* @__PURE__ */ __name((meta) => Object.keys(meta.versions).filter((v) => meta.versions[v]?.dist && !isPreRelease(v)), "getStableVersions");
var fetchPackageMetadata = /* @__PURE__ */ __name(async (pkg) => {
  const res = await fetch(`https://registry.npmjs.org/${pkg}`);
  return res.ok ? res.json() : null;
}, "fetchPackageMetadata");
var parseVersion = /* @__PURE__ */ __name(async (pathname, pkg) => {
...
var buildFullPath = /* @__PURE__ */ __name((base, file) => {
...
function createRoutes() {
  ...
  app.get("/versions.json", async (c) => {
    ...
    if (!meta) return c.json({ error: "Package not found" }, 404);
  ...
  app.get("*", async (c) => {
    ...
    if (!path?.trim()) return c.json({ error: "File path required" }, 400);
    const buildUrl = CDN_URLS[c.env.CDN_PROVIDER];
    if (!buildUrl) return c.json({ error: "Invalid CDN provider" }, 500);
    const cdnUrl = buildUrl(c.env.NPM_PACKAGE, version, buildFullPath(c.env.SCHEMAS_PATH, path));
    try {
      const res = await fetch(cdnUrl);
      if (!res.ok) return c.json({ error: "File not found", url: cdnUrl }, 404);
      const headers = new Headers(res.headers);
      headers.set("Access-Control-Allow-Origin", "*");
      headers.set("Cache-Control", `public, max-age=${c.env.CACHE_MAX_AGE}`);
      return new Response(res.body, { headers });
    } catch (err) {
      return c.json({ error: "CDN fetch failed", message: String(err) }, 502);
    }
  });
  return app;
}
...
// worker.ts
var worker_default = { fetch: createRoutes().fetch };
...
// .wrangler/tmp/bundle-3pP1yl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3pP1yl/middleware-loader.entry.ts
