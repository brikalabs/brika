...
describe('PluginConfigService — secret routing', () => {
  ...
  test('scrubLegacySecrets removes plaintext password values from YAML', async () => {
    configBacking.apiKey = 'leaked-plaintext-key';
    configBacking.serverUrl = 'https://example.com';

    const count = await svc.scrubLegacySecrets({
      hub: {
        port: 3001,
        host: '127.0.0.1',
        plugins: { installDir: '', heartbeatInterval: 0, heartbeatTimeout: 0 },
      },
      plugins: [{ name: PLUGIN, version: '1.0.0', config: configBacking }],
      rules: [],
      schedules: [],
    });
  ...
  test('scrubLegacySecrets removes legacy __oauth_*_token entries from YAML', async () => {
    configBacking.__oauth_spotify_token = { access_token: 'leaked' };
    ...
    expect(count).toBe(1);
    expect(configBacking).not.toHaveProperty('__oauth_spotify_token');
  });

  test('scrubLegacySecrets is a no-op when nothing needs scrubbing', async () => {
    const count = await svc.scrubLegacySecrets({
      hub: {
        port: 3001,
        host: '127.0.0.1',
