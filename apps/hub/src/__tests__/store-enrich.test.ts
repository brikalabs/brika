...
const makeConfig = (installed: Array<{ name: string; version: string }>) => ({
  plugins: installed,
});
...
describe('enrichPlugins', () => {
  test('enriches a list of plugins with install status and compatibility', () => {
    const plugins = [
      {
        package: { name: '@brika/a', version: '1.0.0', engines: { brika: '>=0.1.0' } },
        source: 'npm',
        installVersion: '1.0.0',
        downloadCount: 100,
      },
      {
        package: { name: '@brika/b', version: '2.0.0' },
        source: 'npm',
        installVersion: '2.0.0',
        downloadCount: 50,
      },
    ];
    const config = makeConfig([{ name: '@brika/a', version: '1.0.0' }]);
