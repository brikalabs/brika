...
export class WorkflowLoader {
  ...
  async loadDir(dir: string): Promise<void> {
    ...
    const tomlFiles = await Array.fromAsync(glob.scan({ cwd: this.#dir }));
  ...
  async saveWorkflow(workflow: Workflow): Promise<string> {
    ...
    // Write as TOML
    await Bun.write(filePath, TOML.stringify(workflow as unknown as TOML.TomlPrimitive));
  ...
  async deleteWorkflow(id: string): Promise<boolean> {
    ...
    if (!filePath) {
      ...
      if (this.#dir) {
        const conventionalPath = `${this.#dir}/${id}.toml`;
