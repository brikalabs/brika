...
export const pluginsRoutes = group("/api/plugins", [
  ...
  route.post("/enable", { body: z.object({ ref: z.string() }) }, async ({ body, inject }) => {
    await inject(PluginManager).enable(body.ref);
    return { ok: true };
  }),

  route.post("/disable", { body: z.object({ ref: z.string() }) }, async ({ body, inject }) => {
    await inject(PluginManager).disable(body.ref);
    return { ok: true };
  }),

  route.post("/reload", { body: z.object({ ref: z.string() }) }, async ({ body, inject }) => {
    await inject(PluginManager).reload(body.ref);
    return { ok: true };
  }),

  route.post("/kill", { body: z.object({ ref: z.string() }) }, async ({ body, inject }) => {
