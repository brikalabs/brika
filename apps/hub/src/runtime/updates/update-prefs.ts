 ...
 * readers, and a read-only handle still sees the writer's committed,
 * un-checkpointed frames. When the file or `settings` table is absent
 * (fresh install, hub never started) or the row can't be read, it falls
 * back to the default channel rather than throwing: a missing preference
 * must never block a manual update.
 *
...
/**
 * Read the persisted update channel + pinned version. Never throws: any
 * failure (missing file, locked DB, schema mismatch, corrupt row)
