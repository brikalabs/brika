...
defineRenderer('slider', ({ node, onAction }) => {
  ...
  return (
    <div
      className={cn(
        hasLabel ? 'shrink-0 space-y-1.5 rounded-md bg-muted/40 px-2.5 py-2' : 'shrink-0',
    ...
    >
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, ${resolveColor(node.color) ?? 'var(--color-primary)'} ${pct}%, var(--color-muted) ${pct}%)`,
        }}
      />
    </div>
  );
