...
import { CornerShapeWidget, NumericWidget, SelectWidget, TextWidget, type NumericConfig } from './token-widgets';
...
interface TokenFieldProps {
  ...
  draft: ThemeConfig;
  mode: 'light' | 'dark';
  onColorChange: (token: string, slot: 'light' | 'dark' | 'both', value: string | undefined) => void;
  onTokenChange: (component: string, suffix: string, value: string | undefined) => void;
}

export function TokenField({ spec, draft, mode, onColorChange, onTokenChange }: Readonly<TokenFieldProps>) {
  if (spec.type === 'color') {
    ...
    return <ColorField label={spec.name} value={palette[spec.name] ?? fallback} onChange={(v) => onColorChange(spec.name, 'both', v)} />;
  }
  if (spec.appliesTo) {
    return <NonColorWidget spec={spec} component={spec.appliesTo} draft={draft} onTokenChange={onTokenChange} />;
  }
  return null;
}
...
function NonColorWidget({ spec, component, draft, onTokenChange }: Readonly<NonColorWidgetProps>) {
  ...
  if (spec.type === 'corner-shape') return <CornerShapeWidget value={effective} onChange={setValue} />;
  const selectOpts = SELECT_OPTIONS[spec.type];
  if (selectOpts) return <SelectWidget value={effective} options={selectOpts} onChange={setValue} />;
