      ...
      <Card><CardContent className="pt-6">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input value={ref} onChange={(e) => setRef(e.target.value)} placeholder="@elia/plugin-hue or git+https://..." className="font-mono" />
          </div>
          <div className="space-y-2">
            <Label>Version (optional)</Label>
            <Input value={wanted} onChange={(e) => setWanted(e.target.value)} placeholder="^1.0.0" className="font-mono" />
          </div>
          <div className="flex gap-2 pt-2">
            <Button onClick={handleInstall} disabled={install.isPending || !ref} className="gap-2">
              {install.isPending ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}Install
            </Button>
            <Button variant="destructive" onClick={handleUninstall} disabled={uninstall.isPending || !ref} className="gap-2">
              {uninstall.isPending ? <Loader2 className="size-4 animate-spin" /> : <Trash2 className="size-4" />}Uninstall
            </Button>
          </div>
        </div>
      </CardContent></Card>
