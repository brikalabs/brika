...
export const timerActions = automation("timer-actions")
  ...
  .switch(ctx => ctx.payload.name as string, {
    ...
    "sleep-timer": b => b
      .action("lights.off", { room: "bedroom" })
      .action("tv.off"),
    
    default: b => b
      .log("info", ctx => `Timer "${ctx.payload.name}" completed - no action defined`),
  })
  .build();
...
export const reminderFlow = automation("reminder-flow")
  ...
  .if(ctx => ctx.lastResult === "snooze", b => b
...
export const timerCounter = automation("timer-counter")
  ...
  .if(ctx => (ctx.state.count as number) % 5 === 0, b => b
...
export const nightMode = automation("night-mode")
  ...
  .forEach(ctx => ctx.state.rooms as string[], "room", b => b
    .action("lights.off", ctx => ({ room: ctx.state.room }))
    .delay("500ms") // Stagger to avoid power surge
  )
